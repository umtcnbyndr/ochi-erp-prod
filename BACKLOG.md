...
**2026-07-17 yapılanlar (fiyat/komisyon tutarlılığı + otomatik kâr tabanı):**
- ✅ **Diğer pazaryeri fiyat tabanı marka-bazlı elle multiplier'dan → otomatik "iso-kâr"a geçti.** Diğer pazaryerlerinde rakip görünmediği için fiyat, Trendyol ile AYNI net kârı bırakan tabanın altına inmiyor. Taban komisyon/kargo/stopaj farkından otomatik hesaplanıyor (elle oran yok). Yeni saf fn `computeIsoProfitFloor` (ty-floor.ts): `floor_X = (tyNet + kargoX + ekX)/(1-(komX+stopajX)/100)`, COGS iki tarafta aynı → sadeleşir. dopigo-sync Pass 2 bunu kullanıyor (efektif/kademeli komisyon `resolveEffectiveCommissionSync` ile), GIFT muaf. Test: ty-floor.test.ts +6, 165/165. Prod önizleme (TY=₺5000): HB +%1,2 · PttAvm −%1,6 · N11 −%2,1 · Amazon −%4,7 · Pazarama −%7,8 · Farmazon −%10,8 · Web −%16,4. Sebep: eski sistem marka-marka elle giriliyordu, 14 markadan 1'i doluydu → 13 marka korumasızdı. UI: TY-Floor sekmesi editable'dan **salt-okunur "Kâr Tabanı" bilgi paneline** dönüştü (`ty-floor-info.tsx` + `getAutoFloorPreviewAction`); eski `ty-floor-flow.tsx` + floor action'ları silindi. Commit + deploy 2026-07-17. 🔲 Kalıntı temizlik (ayrı task): `BrandMarketplaceFloor` tablosu + `brand-marketplace-floor.ts` servisi + `applyTrendyolFloor` + validator artık ölü, silinebilir.
- ✅ **Pazar Fiyat Takip taraması SET + GIFT'e açıldı.** `getScanQueue` (market-scan.ts:69) `productType: "SINGLE"` → `{ in: ["SINGLE","SET","GIFT"] }`; PASSIVE hâlâ dışarıda. Prod veri: 73 aktif SET/GIFT'in 72'sinde gerçek TY listing contentId var → kesin taranırlar (çöp barkod araması değil). Tek satır, şema değişmez. **Worker deploy gerekir** (getScanQueue worker'da çalışıyor); sonraki tarama turunda SET/GIFT buybox dolar. Not: SET marjı kartta görünmez (mainPurchasePrice null → sanal maliyet; ayrı iş).
...
- ✅ **Mutabakat Faz 2 tamam (Pazarama + Amazon) + Kanal bug fix + Ay Sonu kaldırıldı.** Pazarama parser (item-bazlı, kampanya/komisyon ×miktar). Amazon parser (İşlem CSV, sipariş çok-satıra dağılır, Transfer/Reklam/Düzeltme sipariş-dışı ayrılır). HB/N11 recon eşleşme anahtarı analytics SQL'de düzeldi (`reconMatchKeySql` tek kaynak). **Kanal breakdown net kârı `recon.other`'ı düşmüyordu** → `resolveChannelExpense` saf fonksiyonu (getChannelBreakdown + calculateChannelExpenses tek kaynak), Kanal sekmesine Diğer kolonu. Ay Sonu UI sekmesi kaldırıldı (fallback+veri kaldı). Detay: memory `multi-marketplace-mutabakat`.
- ✅ **Dopigo Siparişler: Alış'a göre sıralama + drawer inline alış doldurma.** Alış artan → eksik-alış kalemler üste; drawer'da costSource=NONE ise inline doldurma (eşleşmeyen→ManualPurchasePrice, eşleşen→Product.mainPurchasePrice, `saveOrderItemCostAction`). Yan fix: `listOrdersForTable` alış CASE'i ManualPurchasePrice'ı atlıyordu (mpp LATERAL + MANUAL source eklendi, aggregate ile tutarlı). Test channel-expense + recon-match-key. Haziran ~%99.75 gerçek mutabakatlı (kalan kuyruk ay-sonu timing).
- ✅ **Tek P&L motoru + iade maliyeti + HB kredisi (kapsamlı finans denetimi sonrası).** KPI+Kanal artık buildPnlCTE satır toplamlarından (calculateChannelExpenses/resolveChannelExpense/loadRecon* silindi, Ay Sonu fallback analytics'ten tamamen kalktı). İade maliyeti (net≤0 satırların gerçek kargo/ceza, Haz ~7,4K) KPI/Kanal/patron raporu/Gelir-Gider'de düşülüyor; Marka/Kategori'ye dağıtılamaz (bilinçli). HB kredisi (İndirim, ~1,5K/ay) kâra işlendi; TY İndirim'i kredi değil (reconCreditAmount tek kaynak). AÇIK: alış fiyatı tarihçesi yok — mainPurchasePrice güncellenince geçmiş ay kârı değişir; costAtSale snapshot önerisi user kararı bekliyor.
...
- Canlı doğrulama: Trendyol May 797 + Haz 1594, Farmazon Haz 12 → %100 eşleşme, gerçek gider hesabı çalışıyor. Hepsiburada Haz (56, 2026-07-06) ve N11 Haz (37/37, 2026-07-16) canlıya kaydedildi — 4 kanal da yüklü.
...
- ✅ **Pazarama** (2026-07-16): "Siparişleriniz_*.xlsx" item-bazlı; net ciro = Ürün Tutarı − Satıcı Kampanyası (Dopigo ile birebir doğrulandı), komisyon KDV dahil gerçek, Tedarik Edilemedi → net 0 → tam-iade kuralıyla düşer; stopaj raporda yok (tahmin fallback), kargo sipariş-başı input; eşleşme Sipariş Numarası birebir. Not: kampanya/komisyon kolonları ADET BAŞI → ×miktar (a7fd274); satışı olmayan siparişe sabit kargo yazılmaz (1e50b3b).
