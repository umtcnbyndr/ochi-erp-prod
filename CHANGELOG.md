...
- ✅ **Madde 3 — Dopigo Aktarım denetimi: 2 gerçek bug bulundu + düzeltildi.**
  - **Bug 1 — bayatlık kontrolü boşluğu.** `isRecommendationStale` referans olarak `Product.mainPriceUpdatedAt` kullanıyor, ama bu alan SADECE mal kabul (Ürün Giriş, weighted-average) akışından yazılıyordu. Ürün formundan direkt alış düzeltmesi VE Takas (Exchange, hem "stoğa giriş" hem "farklı ürünle takas" dalları) bu alanı hiç güncellemiyordu → o yollardan gelen alış değişiklikleri sonrasında eski öneriler sonsuza kadar "taze" sayılmaya devam ediyordu. Etki alanı: Dopigo Aktarım, **Kupon Önerileri**, **Sipariş Builder/Pazar Fırsatı** (sales-analysis.ts), Ürün detay "bayat" rozeti — hepsi aynı alanı okuyor, tek düzeltme hepsini kapsadı. Fix: `lib/pricing/utils.ts`'e yeni saf fn `purchasePriceChanged` (epsilon-toleranslı, null-aware); `product.ts` (updateProduct+createProduct) ve `exchange.ts` (2 weighted-average sitesi) artık mal kabulle aynı desende `mainPriceUpdatedAt=now()` yazıyor. Test: `utils.test.ts` (11 test). Prod: aktif SINGLE ürünlerin 140/156'sında bu alan hiç set edilmemişti, 95'inde şu an 2+ hafta eski öneri "taze" sayılıyordu.
  - **Bug 2 — SET ürün fiyat hesabı: eczane fallback yoktu + 5 farklı yerde tutarsız kopya mantık vardı.** Kök fonksiyon `calculateSetPurchasePrice` eksik bileşen alışını **sessizce ₺0 sayıyordu** — SET'in gerçek maliyeti olduğundan düşük görünüyor, raporlarda kârı şişiriyordu. Derin tarama 5 ayrı implementasyon buldu, hepsi tek kaynağa indirildi: `lib/pricing/set-product.ts` (ana pure fn, artık `resolveProductUnitCost` ile ana alış>eczane fallback>bloke sırası), `lib/services/set-product.ts` (6 çağrı noktası), `lib/services/dopigo-sync.ts` (canlı Dopigo export), `lib/services/product.ts` (getProductById + ana Ürünler liste "virtualMainPurchasePrice"), `lib/exports/set-products.ts` (Sistem Yedekleme Excel). Yeni dönüş `number | null` (eksik veri BLOKE eder, asla sessizce 0 saymaz) — birkaç UI `.toFixed()` çökme riskini ortaya çıkardı (set-list.tsx, recalculate-button.tsx, set-urun/[id]/page.tsx 6 nokta), hepsi null-güvenli yapıldı; set detay sayfasına "Set fiyatı hesaplanamıyor" bandı eklendi. Test: `set-product.test.ts` +4. Prod: **27 aktif SET** eksik bileşen alışından etkileniyor, hiçbiri "tamamen maliyetsiz" değil (hepsi eczane fallback'i var) → hepsi artık doğru fiyatlanacak.
  - Test: **184/184**, typecheck+lint temiz. Henüz **deploy edilmedi** (bu turda strateji: birkaç maddede bir toplu deploy, kullanıcı onaylı).
