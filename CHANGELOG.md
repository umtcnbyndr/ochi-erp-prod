...
- ✅ **Madde 1 — Sistem Sıfırla + Toplu İsim Düzelt kaldırıldı.** İki tehlikeli/gereksiz admin aracı komple silindi:
  ...
  - Doğrulama: `pnpm typecheck`/`lint` temiz, **167/167 test**, her iki route canlı dev server'da (hot-reload, mutabakat chat'in server'ı) temiz 404 veriyor (500 yok), grep ile hiçbir kalan kod referans vermiyor. Henüz **deploy edilmedi** — bu turda deploy stratejisi "birkaç maddede bir toplu" (kullanıcı onaylı 2026-07-17); commit local'de bekliyor.
